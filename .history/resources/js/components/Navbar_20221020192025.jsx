...
const Navbar = () => {
    const {activeMenu, setactiveMenu, screenSize, setScreenSize, handleClick} = useStateContext()
    ...
    return (
        <div className='flex justify-between p-2 md:mx-6 relative'>
            ...
            <div className='flex '>
                ...
                {dashState.cart && <Cart /> }
                {dashState.chat && <Chat /> }
                {dashState.notification && <Notification /> }
                {dashState.userProfile && <UserProfile /> }
            </div>
        </div>
    );
};
